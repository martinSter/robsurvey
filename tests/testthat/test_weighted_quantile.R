...
test_that("Function outputs correct result for example", {
  ...
  expect_equal(weighted.quantile(x, x, probs = c(0.25, 0.5, 0.75)), c(0.05, 0.15, 0.20), tolerance=1e-05)
})
...
test_that("Weighted quantile is equal to quantile if weights are all 1", {
  ...
  expect_equal(weighted.quantile(x, w, probs), as.vector(quantile(x, probs)), tolerance=1e-05)
})
